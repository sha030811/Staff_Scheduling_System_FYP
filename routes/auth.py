...
def admin_dashboard():
    ...
    if session['role'] == 'doctor':
        # 🔒 Doctor: show only active users in their department
        staff_list = User.query.filter_by(department_id=session['department_id'], is_active=True).all()
    else:
        # 🛡️ Admin: show all active users
...
def add_staff():
    ...
    if request.method == 'POST':
        ...
        if not username:
            ...
            errors['username'] = "Username can only contain letters and numbers (no special characters or spaces)."
        elif User.query.filter_by(username=username).first():
            errors['username'] = "This username is already in use. Choose another one."
        ...
        elif User.query.filter_by(full_name=full_name).first():
            errors['full_name'] = "This full name is already used. Please use a different name."
...
def edit_staff(staff_id):
    ...
    staff_dept_name = staff.department.name
    ...
    if request.method == 'POST':
        ...
        elif User.query.filter(User.username == username, User.id != staff.id).first():
        ...
        if not email:
            ...
            errors['email'] = "Please enter a valid email address."
        elif User.query.filter(User.email == email, User.id != staff.id).first():
            errors['email'] = "This email is already in use."

        # ✅ Role and Department (only admin can change)
        if session['role'] == 'admin':
            role = request.form.get('role')
            dept = request.form.get('department_id')
            if dept == 'Other':
                dept = request.form.get('custom_department_id')
        ...
        if session['role'] == 'admin':
            ...
            if not department_obj:
                ...
                db.session.commit()
            staff.department = department_obj
...
def delete_staff(staff_id):
    ...
    # Restrict doctor from deleting staff outside their department
    if session['role'] == 'doctor' and staff.department_id != session['department_id']:
    ...
    try:
        ...
        db.session.commit()
        flash("🗑️ Staff account deactivated successfully.")
    except Exception as e:
        db.session.rollback()
        flash(f"❌ Deactivation failed: {str(e)}")
...
def shift_schedule():
    ...
    if role == 'admin':
        ...
        staff_list = User.query.filter_by(role='staff', department_id=dept_id, is_active=True).all()
        staff_ids = [s.id for s in staff_list]
        availabilities = Availability.query.filter(Availability.staff_id.in_(staff_ids)).order_by(Availability.date).all()
    else:
        flash("⛔ Access denied.")
...
def get_shifts():
    ...
    if role == 'admin':
        active_staff_ids = [u.id for u in User.query.filter_by(is_active=True).all()]
        shifts = Shift.query.filter(Shift.staff_id.in_(active_staff_ids)).all()
    elif role == 'doctor':
        dept_id = session.get('department_id')
        staff_ids = [u.id for u in User.query.filter_by(department_id=dept_id, is_active=True).all()]
...
def schedule_requests():
    ...
    if role == 'doctor':
        dept_id = session.get('department_id')
        staff_list = {u.id: u for u in User.query.filter_by(role='staff', department_id=dept_id, is_active=True).all()}
        staff_ids = list(staff_list.keys())
        query = query.filter(ScheduleRequest.staff_id.in_(staff_ids))
    elif role == 'admin':
        staff_list = {u.id: u for u in User.query.filter_by(role='staff', is_active=True).all()}
    else:
        flash("⛔ Access denied.")
        return redirect('/login')
...
def update_availability():
    if request.method == 'POST':
        ...
        if not staff:
            ...
            return redirect('/login')
        doctor = User.query.filter_by(role='doctor', department_id=staff.department_id, is_active=True).first()
        recipient = doctor or User.query.filter_by(role='admin', is_active=True).first()
...
def view_availability_requests():
    ...
    if session['role'] == 'admin':
        users = User.query.filter_by(role='staff', is_active=True).all()
    elif session['role'] == 'doctor':
        dept_id = session.get('department_id')
        users = User.query.filter_by(role='staff', department_id=dept_id, is_active=True).all()
    else:
        flash("⛔ Access denied.")
        return redirect('/login')
   ...
   # 🟩 Determine staff who submitted availability for this week
    ...
    # 🟩 All staff IDs
    all_staff_ids = {u.id for u in User.query.filter_by(role='staff', is_active=True).all()}
...
def view_all_availability():
    ...
    if role == 'admin':
        staff_query = User.query.filter_by(role='staff', is_active=True)
    elif role == 'doctor':
        staff_query = User.query.filter_by(role='staff', department_id=dept_id, is_active=True)
    else:
        flash("⛔ Access denied.")
        return redirect('/login')
...
def submit_request():
    ...
    if request.method == 'POST':
        ...
        staff = User.query.filter_by(id=staff_id, is_active=True).first()
        if not staff:
            flash("⚠️ Invalid or deactivated staff account.", "danger")
            return redirect('/login')
        ...
        if staff.role == 'staff':
            ...
            if doctor:
                ...
                recipient = User.query.filter_by(role='admin', is_active=True).first()
        else:
           recipient = User.query.filter_by(role='admin', is_active=True).first()
...
def resubmit_availability(req_id):
    ...
    if request.method == 'POST':
        ...
        # ✅ Now properly indented: notify doctor/admin
        staff = User.query.filter_by(id=old_req.staff_id, is_active=True).first()
        if staff:
            doctor = User.query.filter_by(role='doctor', department_id=staff.department_id, is_active=True).first()
            recipient = doctor if doctor else User.query.filter_by(role='admin', is_active=True).first()
            ...
            if recipient:
                ...
                notification = Notification(
                    sender_id=staff.id,
                    recipient_id=recipient.id,
                    title="Resubmitted Availability",
                    message=f"{staff.full_name} has resubmitted availability for {old_req.date} ({old_req.day_of_week}).",
                    priority="Normal"
                )
                db.session.add(notification)
                ...
                if recipient.email:
                    try:
                        msg = Message(
                            subject="Availability Resubmission Notification",
                            sender=current_app.config['MAIL_USERNAME'],
                            recipients=[recipient.email],
                            body=f"""Dear {recipient.full_name},

{staff.full_name} has resubmitted their availability for:

📅 Date: {old_req.date} ({old_req.day_of_week})  
⏰ New Time: {new_time}
...
@auth_bp.route('/export_shift_csv')
def export_shift_csv():
    ...
    output = StringIO()
    writer = csv.writer(output)
    writer.writerow(['Department', 'Date', 'Shift Time', 'Required Staff', 'Assigned Staff', 'Coverage Status'])
    ...
    required = 3
    for r in results:
        status = "Full" if r.assigned_staff >= required else "Partial" if r.assigned_staff >= 1 else "Unstaffed"
        writer.writerow([r.department, r.date, r.time, required, r.assigned_staff, status])

    output.seek(0)
    return Response(output, mimetype="text/csv",
                    headers={"Content-Disposition": "attachment;filename=shift_coverage_report.csv"})


@auth_bp.route('/export_shift_pdf')
def export_shift_pdf():
    ...
    pdf.setFont("Helvetica-Bold", 14)
    pdf.drawString(1 * inch, height - 1 * inch, "Shift Coverage Report")
    ...
    pdf.setFont("Helvetica", 10)
    y = height - 1.5 * inch
    pdf.drawString(0.7 * inch, y, "Department")
    pdf.drawString(2.7 * inch, y, "Date")
    pdf.drawString(3.7 * inch, y, "Shift")
    pdf.drawString(4.7 * inch, y, "Required")
    pdf.drawString(5.7 * inch, y, "Assigned")
    pdf.drawString(6.7 * inch, y, "Status")

    y -= 15
    for row in results:
        status = (
            "Full" if row.assigned_staff >= required_per_shift else
            "Partial" if row.assigned_staff >= 1 else
            "Unstaffed"
        )
        pdf.drawString(0.7 * inch, y, row.department or "Unknown")
        pdf.drawString(2.7 * inch, y, row.date)
        pdf.drawString(3.7 * inch, y, row.time)
        pdf.drawString(4.7 * inch, y, str(required_per_shift))
        pdf.drawString(5.7 * inch, y, str(row.assigned_staff))
        pdf.drawString(6.7 * inch, y, status)
    ...
    pdf.save()
    buffer.seek(0)
    return send_file(buffer, as_attachment=True, download_name='shift_coverage_report.pdf', mimetype='application/pdf')

@auth_bp.route('/export_shift_xlsx')
def export_shift_xlsx():
    ...
    output = BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df.to_excel(writer, index=False, sheet_name='Shift Coverage')

    output.seek(0)
    return send_file(output,
                     download_name=f"shift_coverage_export_{date.today()}.xlsx",
                     as_attachment=True,
                     mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
def staff_list():
    ...
    query = User.query.filter_by(is_active=False) if show_inactive else User.query.filter_by(is_active=True)

    if role == 'admin':
        if selected_dept:
            query = query.filter(User.department.has(name=selected_dept))
        if selected_role:
        ...
        departments = [d.name for d in db.session.query(Department.name).distinct()]
        ...
        dept_id = session.get('department_id')
        staff = query.filter_by(role='staff', department_id=dept_id).all()
        doctors = []
        admins = []
...
def login_activity():
    ...
    for entry in unlinked_fails:
        ...
        matched_user = User.query.filter_by(username=attempted, is_active=True).first()
...
@auth_bp.route('/notifications/unread_count')
def unread_count():
    user_id = session.get('user_id')
    if not user_id:
...
def mark_read(notification_id):
    ...
    if not notification:
        return jsonify({'error': 'Notification not found'}), 404
    notification.is_read = True  # ✅ FIXED
    db.session.add(notification)  # ✅ ensure it’s tracked
    db.session.commit()
    return jsonify({'status': 'success'})

@auth_bp.route('/reactivate_staff/<int:staff_id>')
def reactivate_staff(staff_id):
    ...
    # Doctors can only reactivate staff from their own department
    if session['role'] == 'doctor' and staff.department_id != session['department_id']:
        flash("⛔ You cannot reactivate staff from another department.", "danger")
...
Your account in the Hospital Staff Scheduling System has been successfully reactivated.
...
"""
    ...
    flash(f"🟢 {staff.full_name}'s account has been reactivated and notified via email.", "success")
    return redirect(url_for('auth.staff_list'))
